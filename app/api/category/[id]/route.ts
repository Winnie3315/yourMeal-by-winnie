...
export const PATCH = async (req: NextRequest, { params }: {params : {id: string}}) => {
    try {
        ...
        const categoryItems = await db.collection("category").findOneAndUpdate({_id: new ObjectId(params.id)}, { $set: body})
        return NextResponse.json({ success: true, data: categoryItems });
    } catch (e: any) {
        return NextResponse.json({ success: false, message: e.message }, { status: 500 });
    }
}
...
export const DELETE = async (req: NextRequest, { params }: {params : {id: string}}) => {
    try {
        ...
        const categoryItems = await db.collection("category").findOneAndDelete({_id: new ObjectId(params.id)})
        return NextResponse.json({ success: true, data: categoryItems });
    } catch (e: any) {
        return NextResponse.json({ success: false, message: e.message }, { status: 500 });
    }
...
export const GET = async (res: NextResponse, req: NextRequest) => {
    try {
        const client = await clientPromise;
        const db = client.db("mydatabase");
        const categoryItems = await db.collection("category").find({}).toArray()
